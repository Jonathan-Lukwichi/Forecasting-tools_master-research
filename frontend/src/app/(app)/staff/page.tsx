    ...
    <div className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900">Staff Planner</h1>
            <p className="mt-1 text-sm text-slate-500">MILP-optimized staff scheduling</p>
          </div>
          <button onClick={() => router.push("/supply")} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600 hover:border-slate-300 hover:bg-slate-50">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mb-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <input type="number" min={1} max={30} value={horizon} onChange={(e) => setHorizon(Number(e.target.value))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 focus:border-blue-400 focus:outline-none" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-slate-600">Min Staff / Shift</label>
              <input type="number" min={1} max={20} value={minStaff} onChange={(e) => setMinStaff(Number(e.target.value))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 focus:border-blue-400 focus:outline-none" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-slate-600">Max Overtime (hrs)</label>
              <input type="number" min={0} max={12} value={maxOT} onChange={(e) => setMaxOT(Number(e.target.value))} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 focus:border-blue-400 focus:outline-none" />
            </div>
          </div>
          <button onClick={handleOptimize} disabled={!selectedId || loading} className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-sky-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm disabled:opacity-40">
        ...
        {result && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                { label: "Status", value: result.status, icon: <CheckCircle2 size={16} className={result.is_optimal ? "text-emerald-500" : "text-amber-500"} /> },
                { label: "Total Cost", value: `$${result.total_cost.toLocaleString()}`, icon: <DollarSign size={16} className="text-blue-600" /> },
                { label: "Solve Time", value: `${result.solve_time}s`, icon: <Clock size={16} className="text-violet-500" /> },
                { label: "Variables", value: result.num_variables.toLocaleString(), icon: <Users size={16} className="text-blue-500" /> },
              ].map((kpi) => (
            ...
            <div className="grid gap-4 sm:grid-cols-4">
              {[
                { label: "Regular Labor", value: result.regular_labor_cost, color: "text-blue-600" },
                { label: "Overtime", value: result.overtime_cost, color: "text-amber-500" },
                { label: "Understaffing", value: result.understaffing_penalty, color: "text-red-500" },
                { label: "Overstaffing", value: result.overstaffing_penalty, color: "text-orange-500" },
              ].map((c) => (
            ...
            {dailyData.length > 0 && (
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <h3 className="mb-3 text-sm font-bold text-slate-800">Daily Staff Allocation</h3>
